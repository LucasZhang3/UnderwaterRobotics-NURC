...
#define TELEMETRY_TIMEOUT_MS 500    // Maximum age of depth telemetry before depth hold is disabled.
// ----------------- Motor configuration ------------------ //
...
/*
Purpose:
Validates and decodes one bottom-side telemetry packet.
Inputs:
Null-terminated V... telemetry string from RS-485.
Outputs:
Updates volts[] plus filtered depth telemetry in feet; returns 0 when valid.
Side Effects:
Refreshes depth telemetry validity and timestamp used by depth hold.
Dependencies:
Bottom-side telemetry packet field count and width must match this parser.
Notes:
Invalid packets are rejected so stale or malformed data cannot drive depth hold.
*/
...
/*
Purpose:
Normalizes D-pad direction input across Xbox-style buttons, HID button bits, and hat axes.
Inputs:
Global buttons[], butts bitfield, and USB joystick axes.
Outputs:
Boolean up/down/left/right direction flags.
Side Effects:
None.
Dependencies:
USBHost joystick objects must have current input data.
Notes:
Keeps LED and camera controls usable across controller models.
*/
...
/*
Purpose:
Ramps virtual axes generated from D-pad and face-button presses.
Inputs:
Button direction states and current analogs[] values.
Outputs:
Updated analogs[] entries for D-pad and face-button virtual axes.
Side Effects:
Maintains virtual axis state between loop iterations.
Dependencies:
readDpadDirections() and button_inc.
Notes:
Used for LED brightness, camera tilt, and existing button-axis behaviors.
*/
...
/*
Purpose:
Limits how quickly one command channel can move toward its target value.
Inputs:
Current value, target value, acceleration rate, deceleration rate, and elapsed time.
Outputs:
Rate-limited command value.
Side Effects:
None.
Dependencies:
Called before thruster mixing.
Notes:
Deceleration is intentionally faster than acceleration for quick stopping.
*/
...
/*
Purpose:
Detects the LB+RB press edge and toggles slow mode.
Inputs:
Current button state and previous combo latch.
Outputs:
Updates slow_mode_enabled.
Side Effects:
Refreshes LCD mode status when slow mode changes.
Dependencies:
buttons[] must contain the latest controller button state.
Notes:
Edge detection prevents repeated toggles while LB+RB remain held.
*/
...
/*
Purpose:
Applies mode-dependent slew limiting and slow-mode scaling to motion axes.
Inputs:
Requested motion axes and elapsed loop time.
Outputs:
Updated analogs[] motion axes used by thruster mixing.
Side Effects:
None outside the command arrays.
Dependencies:
limitRateOfChange(), slow_mode_enabled, and slew-rate constants.
Notes:
Gripper, LED, and camera controls are left unsmoothed for direct actuator response.
*/
...
/*
Purpose:
Forces all pilot-generated command channels back to neutral when the USB controller is not present.
Inputs:
None.
Outputs:
Zeros normalized control arrays and switch states.
Side Effects:
Disables depth hold so automatic vertical output cannot continue after disconnect.
Dependencies:
Called before command mixing when joystickConnected() is false.
Notes:
Prevents stale joystick, trigger, D-pad, or button values from reaching the command packet.
*/
void clearPilotCommands(void) {
  for (int i = 0; i < NANALOG; i++) {
    analogs[i] = 0.0;
    requestedAnalogs[i] = 0.0;
  }
...
void updateDepthHoldAssist(float dtSeconds) {
  ...
  if (!joystickConnected() || !depthTelemetryValid || (millis() - lastTelemetryMillis > TELEMETRY_TIMEOUT_MS)) {
  ...
  if (Serial1.available()) {
    ...
    if (inChr) {
      if (inPtr < reply_msg + sizeof(reply_msg) - 2) {
        *inPtr++ = inChr;   // store it if it's not null
      } else {
        inPtr = reply_msg;
        depthTelemetryValid = false;
        disableDepthHold();
...
// newline indicates the message is complete, so prepare it for action
  ...
  if (gotInString) {
    Serial.print(reply_msg);        // mirror received telemetry for operator/debug visibility
    gotInString = false;
    if (parse_reply_msg(reply_msg) == 0) {
