...
#define COMMAND_TIMEOUT_MS 500     // Stop critical actuators if no valid command packet arrives in this window.
#define ESC_ARM_DELAY_MS 2000      // Neutral ESC settle time at boot; avoids startup current spikes.
#define ENFORCE_COMMAND_CHECKSUM 0 // Set to 1 after both boards are confirmed to use matching packet code.
#define GRIP_BRAKE_PWM 1           // Existing neutral gripper command: both H-bridge sides held at brake/idle.
#define GRIP_PWM_MIN 0             // Prevent negative PWM values from wrapping into unsafe outputs.
#define GRIP_PWM_MAX 255           // Keep gripper PWM inside analogWrite's 8-bit command range.
...
Notes:
Checksum comparison can be enabled after bring-up confirms both boards are running matching packet code.
*/
int parse_command_msg(char *msg, int *mots, int *sers) {
	...
	if ((val = atox(msg, 2)) < 0) return 1;
	msg += 2;
#if ENFORCE_COMMAND_CHECKSUM
	if ((unsigned char)checksum != (unsigned char) val) return 1;
#endif
	if (*msg++ != '\n') return 1; 
	if (*msg++ != '\0') return 1; 
	return 0;
...
void build_reply_msg(char *msg) {
	...
	*msg++ = 'V';		// for volts
	...
	// Telemetry reply preserves the existing V... newline protocol; command packets carry the safety checksum.
  *msg++ = '\n'; 
	*msg = '\0';
}
...
/*
Purpose:
Clamps gripper PWM values before analogWrite().
Inputs:
Integer PWM command.
Outputs:
PWM value limited to the valid 0..255 range.
Side Effects:
None.
Dependencies:
GRIP_PWM_MIN and GRIP_PWM_MAX define the allowed gripper driver command range.
Notes:
Prevents negative gripper math from wrapping into unsafe high PWM output.
*/
int clamp_grip_pwm(int cmd) {
  if (cmd < GRIP_PWM_MIN) return GRIP_PWM_MIN;
  if (cmd > GRIP_PWM_MAX) return GRIP_PWM_MAX;
  return cmd;
}

/*
Purpose:
Immediately places safety-critical actuators into their neutral/off state.
Inputs:
None.
Outputs:
Neutral thruster pulses and gripper brake/idle PWM.
Side Effects:
Stops active thruster and gripper commands.
Dependencies:
Servo objects and gripper pins must already be initialized.
Notes:
Used when command packets are invalid or the command link times out.
*/
void stop_all_actuators(void) {
  stop_all_motors();
  analogWrite(GRIP1_PIN, GRIP_BRAKE_PWM);
  analogWrite(GRIP2_PIN, GRIP_BRAKE_PWM);
}

/*
Purpose:
Runs the required ESC arming sequence once during startup.
Inputs:
None.
Outputs:
ESCs receive neutral command pulses while they finish booting.
Side Effects:
Blocks briefly during startup while ESCs settle.
Dependencies:
Thruster Servo objects must already be attached.
Notes:
Avoids the old all-motors-forward pulse that could brown out the robot power bus.
*/
void runEscStartupSequence(void) {
  ...
  Serial.println("ESC neutral arm done");
#endif
}

/*
Purpose:
Applies the most recently parsed command packet to thrusters, gripper, camera tilt, and LED dimming.
Inputs:
Global motors[] and servos[] command arrays decoded from a valid packet.
Outputs:
Writes ESC servo pulses, gripper PWM, camera servo pulse, and LED PWM.
Side Effects:
Directly changes physical actuator outputs.
Dependencies:
parse_command_msg() must validate the packet before this function is called.
Notes:
Camera tilt uses the attached Servo object; LED dimming remains analogWrite PWM.
*/
void applyActuatorCommands(void) {
  ...
  analogWrite(GRIP1_PIN, clamp_grip_pwm(grip1));
  analogWrite(GRIP2_PIN, clamp_grip_pwm(grip2));
...
Notes:
Invalid or missing valid command packets force safety-critical actuators to neutral.
*/
void loop() {		
	char st = 0;
  ...
  if (lastGoodPacketMs && (millis() - lastGoodPacketMs > COMMAND_TIMEOUT_MS)) {
    stop_all_actuators();
  }
}
