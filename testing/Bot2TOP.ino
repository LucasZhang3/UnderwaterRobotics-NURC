...
#define VERTICAL_DEADBAND 0.05
#define DEPTH_HOLD_ACTIVATION_DELAY_MS 500
#define DEPTH_HOLD_DEADBAND 0.15
#define MAX_VERTICAL_PID_OUTPUT 0.35
#define DEPTH_KP 0.45
#define DEPTH_KI 0.02
#define DEPTH_KD 0.10
#define DEPTH_UNITS_SCALE 1.0
...
float currentDepthMeters = 0.0;
float filteredDepthMeters = 0.0;
float targetDepthMeters = 0.0;
...
void display_pid_hold_line(void) {
  sprintf(buf, "PID:%+5.2f ft %3s", targetDepthMeters - filteredDepthMeters,
...
void display_all_telems(void) {
  ...
  depthDisplayFt = filteredDepthMeters - DEPTH_DISPLAY_OFFSET_FT;
...
int parse_reply_msg(char *msg) {
  ...
  if (depthTelemetrySampleValid(currentDepthMeters, volts[5])) {
...
void updateDepthHoldAssist(float dtSeconds) {
  ...
  if (!depthTelemetrySampleValid(filteredDepthMeters, volts[5])) {
    disableDepthHold();
    return;
  }
